...
var isCompanyCoordinator = "false";
...
var loadSuggestion = function() {
    var loadSuggestionCallback = {
        success: function(o) {
            var suggestion = YAHOO.lang.JSON.parse(o.responseText).getTeamraiserSuggestionResponse.suggestion;
            // Clear any existing click listeners
            YAHOO.util.Event.removeListener("what-next-answer", "click");
            // Unhide
            show_pc2_element("bd-what-next");
            var el = YAHOO.util.Dom.get("what-next-answer");
            // Replace message
            el.innerHTML = suggestion.message;

            if(suggestion.type == "GOAL") {
                YAHOO.util.Event.addListener(el, "click", showGoalEdit);
            }
            if(suggestion.type == "PERSONAL_PAGE") {
                YAHOO.util.Event.addListener(el, "click", function() {
                    //window.location.href="personalpageinfo.html";
                    YAHOO.Convio.PC2.Utils.loadView("personalpage","compose");
                });
            }
            if(suggestion.type == "CONTACTS") {
                YAHOO.util.Event.addListener(el, "click", function() {
                    //window.location.href="contacts.html";
                    YAHOO.Convio.PC2.Utils.loadView("email","contacts");
                });
            }
            if(suggestion.type == "COMPOSE") {
                YAHOO.util.Event.addListener(el, "click", function() {
                    //window.location.href="contacts.html";
                    YAHOO.Convio.PC2.Utils.loadView("email","compose");
                });
            ...
            if(suggestion.type == "NONE") {
                YAHOO.util.Dom.addClass(el,"what-next-none");
            }

            YAHOO.Convio.PC2.Utils.publisher.fire("pc2:suggestionLoaded", suggestion);
        },
        failure: function(o) {
...
var loadRecentActivity = function() {
    ...
    YAHOO.Convio.PC2.Component.RecentActivity.columnDefs = [
        {key:"type", label:" ", /*className:'delete-button',*/ formatter: YAHOO.Convio.PC2.Component.RecentActivity.formatType}, 
        {key:"activity", label:" ", className: "recent-activity-detail"},
        {key:"followup", label:" ", className: "recent-activity-followup", formatter: YAHOO.Convio.PC2.Component.RecentActivity.formatFollowup},
        {key: "date", label:" ", className: "recent-activity-date"}
    ];
...
var GetPersonalDonationByDayCallback = {
    success: function(o) {
	    ...
	    if(YAHOO.util.SWFDetect.isFlashVersionAtLeast(9.045)){
			...
			var    str = YAHOO.Convio.PC2.Utils.formatCurrency(item.amount*100) + " donated on\n" + item.date;
			return str; 
		    } 
...
function loadComponents() {
    ...
    YAHOO.Convio.PC2.Views.load["dashboard"] = function(subview) {
        ...
        if(subview == "home") {
            if(YAHOO.Convio.PC2.Views.dashboardReset) {
                YAHOO.Convio.PC2.Views.dashboardReset = false;
        	...
        	Y.use("pc2-compose-functions", "pc2-contacts-functions",function(Y) {
        		if(!YAHOO.Convio.PC2.Views.emailCompose) {
                    ...
                    YAHOO.Convio.PC2.Utils.require("pc2:configurationLoaded", "pc2:registrationLoaded", "pc2:allContactsLoaded", function() {
                        ...
                        YAHOO.util.Event.addListener("msg_cat_email_compose_use_template_label", "click", showTemplates);
                        YAHOO.util.Event.addListener("msg_cat_email_compose_hide_template_label", "click", hideTemplates);
                ...
                Y.use("pc2-contacts-functions", function(Y) {
                    ...
                    YAHOO.Convio.PC2.Utils.require("pc2:configurationLoaded", "pc2:registrationLoaded", function() {
                       ...
                       */
                        ...
                        * Plaxo
                        */
                        var showPlaxoPage = function() {
                            showPlaxoABChooser('plaxo_contacts', YAHOO.Convio.PC2.Config.getSecurePath() + '../plaxo/plaxo.html');
                            return false;
                        };

                        YAHOO.util.Event.addListener(
                                   "msg_cat_contacts_import_contacts",
                                   "click", 
                                   showPlaxoPage, 
                                   null, 
                                   false);
    ...
    YAHOO.Convio.PC2.Views.dashboardReset = true;
    //YAHOO.Convio.PC2.Utils.loadView("dashboard", "home");
}
